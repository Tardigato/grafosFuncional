...
// Function to calculate the assignment algorithm
function calcularAsignacion() {
  ...
  const m = nodos.length; // Assuming equal number of workers and tasks for simplicity

  // Initialize the cost matrix with zeros
  const costMatrix = new Array(n).fill().map(() => new Array(m).fill(0));

  // Iterate through the edges to assign costs
  aristas.forEach(arista => {
      const fromIndex = nodos.findIndex(nodo => nodo.id === arista.from);
      const toIndex = nodos.findIndex(nodo => nodo.id === arista.to);
      const cost = parseInt(arista.label || 0); // Assuming the label contains the cost
      costMatrix[fromIndex][toIndex] = cost;
  });

  // Implementación del algoritmo de asignación (Hungarian Algorithm)
  const rowCover = new Array(n).fill(false);
  const colCover = new Array(m).fill(false);
  const labels = new Array(n).fill(0);
  const minSlack = new Array(m).fill(Infinity);
  ...
  while (assigned < n) {
      let row = -1, col = -1, minVal = Infinity;
      for (let i = 0; i < n; i++) {
          if (!rowCover[i]) {
              for (let j = 0; j < m; j++) {
                  if (!colCover[j]) {
                      const slack = costMatrix[i][j] - labels[i] - labels[j];
                      if (slack < minSlack[j]) {
                          minSlack[j] = slack;
                          parent[j] = i;
                      }
                      if (minSlack[j] < minVal) {
                          minVal = minSlack[j];
                          row = parent[j];
                          col = j;
                      }
  ...
  for (let j = 0; j < m; j++) {
      if (parent[j] !== -1) {
          assignments.push([parent[j], j]);
      }
  }

  const resultadosDiv = document.getElementById('resultadosAsignacion');
  let resultadosHTML = "<h2>Resultados del Algoritmo de Asignación</h2>";
  resultadosHTML += "<p>Asignaciones:</p><ul>";
  for (const [i, j] of assignments) {
      resultadosHTML += `<li>Tarea ${i + 1} asignada a Trabajador ${j + 1}</li>`;
  }
  resultadosHTML += "</ul>";
